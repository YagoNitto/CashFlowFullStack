...
public partial class ListCategoriesPage : ComponentBase
{
    ...
    public async void OnDeleteButtonClicked(Guid id, string title)
    {
        var result = await DialogService.ShowMessageBox("ATENÇÃO", 
            $"Ao Proseguir a categoria {title} será excluída. Está é uma ação irreversível! Deseja continuar?",
            yesText: "EXCLUIR",
            cancelText: "Cancelar");
    ...
    public async Task OnDeleteAsync(Guid id, string title)
    {
        try
        {
            ...
            Snackbar.Add($"Categoria {title} excluída com sucesso.", severity: Severity.Success);
        }
        catch (Exception ex)
        {
            Snackbar.Add(ex.Message, Severity.Error);
